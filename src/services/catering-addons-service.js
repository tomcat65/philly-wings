  ...
  beverages.forEach(item => {
    if (item.sourceDocumentId && item.flavorVariant && item.packSize) {
      ...
      if (!grouped[groupingKey]) {
        // Extract base name and add pack size only if not already present
        const baseName = item.name ? item.name.replace(/\s*\(.*?\)\s*/g, '').trim() : 'Boxed Iced Tea';

        // Check if pack size info is already in the base name
        const packSizeInfo = item.packSize || '';
        const hasPackSizeInName = baseName.toLowerCase().includes(packSizeInfo.toLowerCase().replace(/\s*-.*$/, ''));

        const displayName = hasPackSizeInName ? baseName : `${baseName} (${item.packSize})`;

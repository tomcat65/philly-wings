      ...
      <div class="template-content">
        ...
        ${template.stats?.avgRating ? `
          <div class="template-rating">
            <span class="rating-stars">${'★'.repeat(Math.round(template.stats.avgRating))}${'☆'.repeat(5 - Math.round(template.stats.avgRating))}</span>
            <span class="rating-value">${template.stats.avgRating.toFixed(1)}</span>
          </div>
        ` : ''}
...
export function initTemplateSelector(onTemplateSelect) {
  ...
  // Re-query the new elements after cloning
  const freshTemplateCards = document.querySelectorAll('.template-card');
  const freshSelectButtons = document.querySelectorAll('.btn-template-select');

  // Card click handlers (on fresh elements)
  freshTemplateCards.forEach((card, index) => {
  ...
  // Button click handlers (on fresh elements)
  freshSelectButtons.forEach((button, index) => {
